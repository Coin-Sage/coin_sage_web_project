...
def preprocess(dataset, cfg, logger=None):
    ...
    if ('dataset_loader' not in cfg
            or
            'train_start_date' not in cfg['dataset_loader']
            or
            'valid_end_date' not in cfg['dataset_loader']):
    ...
    features = cfg['dataset_loader'].get('features', dataset.columns)
    if 'Date' in features:
    ...
    indicators_names = list(
        cfg['dataset_loader'].get('indicators_names', '').split(' ')
    )
    ...
    profit_calculator.to_csv('profit_calculator.csv', index=False)
